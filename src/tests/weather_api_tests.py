...
class SunPowerInDifferentStates(unittest.TestCase):
    ...
    def test_sun_power_in_our_shining_time(self):
        for _ in range(1000):
            ...
            if 300 < self.world.daytime < 1140:
                self.assertTrue(0 < self.world.weather['sun'] <= 1,
                                "Sun should shining now.")
        ...
        for _ in range(1000):
            ...
            if not (300 <= self.world.daytime <= 1140):
                self.assertTrue(self.world.weather['sun'] == 0,
                                "Sun should not shining now.")

    def test_wind_power_in_different_stapes(self):
        for _ in range(1000):
            self.world.step()

            self.assertTrue(0 <= self.world.weather['wind'] <= 1,
                            "Wind power incorrect value.")
        ...
        for _ in range(1000):
            ...
            if self.world.weather['wind']:
                self.assertTrue(0 <= self.world.weather['clouds'] < 0.6,
                                "Clouds value calculated with wind incorrect.")
    ...
    def test_rain_not_falling(self):
        ...
        for value in temp_list:
            ...
            self.assertEqual(self.world.weather['rain'], 0,
                             "Incorrect rain value - rain should be not falling.")
    ...
    def test_rain_falling(self):
        ...
        for value in temp_list:
            self.world.weather['clouds'] = value
            self.world._calculate_rain()
            self.assertEqual(self.world.weather['rain'], 1,
                             "Incorrect rain value - rain should be falling.")

    def test_light_value(self):
        self.world.weather['sun'] = 0.4
        self.world.weather['clouds'] = 0.5
        self.world._calculate_light()
        self.assertEqual(self.world.weather['light'], 0.2,
    ...
    def test_weather_meaning_scale(self):
        self.assertEqual(
            self.world.previous_weather_meaning +
            self.world.current_weather_meaning,
            1,
