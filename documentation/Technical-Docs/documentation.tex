...
Besides the Reinforcement Learning field's recent, exciting progress, there is even more motivation behind testing it's methods in the decision making processes:
\begin{itemize}
\item Systems based on RL need much less expert knowledge to work properly.
\item Models can find optimal, 'golden' points, which is hard to achieve by a human operator. 
\item There is a big potential of making the models transferable between environments that share their profile, but are different in details, speeding the deployment process by eliminating the necessity to adjust system's modules.
\item Optional possibility of using these models as supportive decision recommenders, not eliminating but \textbf{improving} the human performance.
\end{itemize}

One of the main advantage of such solution to the problem of, for example, managing a house, is that there is very little expert knowledge needed for these methods to learn. Of course, the reward signal is usually shaped by human, but shaping it is usually much less complicated, then translating a full set of rules (often formulated in quite fuzzy natural language) into the computer programming language. It is important to note here, that some of the recent achievements of RL are based on fully sparse rewards - like binary signals, where the signal's value is 1 in case of success and -1 in case of failure. Simple reward formulations, like \textit{distance}, that model traveled using a simulated human body in a fixed amount of time, are usually enough to learn an optimal running strategy. The ability to learn optimal strategies with so little knowledge is really impressive. This opens new possibilities, with software that, quite possibly in future, is ready to adapt itself (and find an optimal way to do it) in real life scenarios of decision making processes. \

\subsection{Potential applications}
In our project, we used Reinforcement Learning methods to successfully manage a simulated house environment. Based on the research and our own ideas, here we list some other example applications of these methods to real-life problems:

\begin{itemize}
\item \textbf{Scheduling problems.} This is family of problems that contain both the low level world of scheduling processes in the operating systems, and high-level world of possible scheduling problems in automatic storage units, factories and transport/logistics.
\item \textbf{Resources distribution.} For example, problems like finding optimal distribution of human resources in big construction sites.
\item \textbf{Attention mechanisms.} Use of Reinforcement Learning in attention-based problems is already proved to work, where the agent help a computer vision system choose where are the important fragments of the video or image \cite{attention}.
\item \textbf{Trading systems.} While it is hard to find any optimistic results, there are claims and hopes, that these methods could work in highly stochastic environments. Interesting approaches make use of web scraping tools with NLP units to find hidden patterns and connections of, for example, tendency of cryptocurrency value and social media posts that mention it's name.
\end{itemize}
...
\bibitem{attention}
	\textit{Visual Attention Model in Deep Learning}: https://towardsdatascience.com/visual-attention-model-in-deep-learning-708813c2912c
